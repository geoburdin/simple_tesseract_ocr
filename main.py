...
from my_tesseract import convert_pdf_to_image, run_tesseract
from prepare_image import process_image_for_ocr


@click.command()
@click.option('--input',
              help='Input Filememe. Supported extensions: pdf, jpg, png, jpeg')
@click.option('--output',
              default='result.txt',
              help='Filename for output: Supported extensions: txt, pdf, hocr')
...
def pipeline(input, output, verbose):
    ...
    if tail[-4:] == '.pdf':
        ...
        try:
            os.mkdir('png_pages')
            logging.warning(
                'Created /png_pages/ folder with pages in png format')
        ...
        for page in glob('png_pages/*.png'):
            ...
            logging.info(
                'Page preprocessing is completed. Result: {}'.format(page))
            run_tesseract('{}.png'.format(page[:-4]),
                          'output',
                          lang='eng',
                          extension='txt',
                          config='--psm 3 --oem 3')
        ...
        logging.info(
            'Processing pdf file is completed. Post-correction is starting')
        ...
        logging.info(
            'Processing input file is completed. Post-correction is starting')
    else:
        logging.error(
            'Input file format is not supported. Supported extensions: pdf, jpg, png, jpeg'
        )
